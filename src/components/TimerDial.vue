  ...
  <div class="aspect-square w-[clamp(264px,76vw,420px)]">
    ...
    >
      ...
      <div class="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-1.5">
        ...
        <span
          class="text-foreground font-semibold tabular-nums leading-none tracking-[-0.02em] text-[clamp(3.4rem,15vw,5.4rem)]"
        >
          {{ time }}
        </span>
        <span class="text-muted min-h-[1em] text-[clamp(0.66rem,2.2vw,0.78rem)] uppercase tracking-[0.16em]">
          {{ statusLabel }}
        </span>
      </div>
    </div>
